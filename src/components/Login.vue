<template>
  <div class="login-wrapper">
    ...
    <form @submit.prevent="submit" class="d-flex flex-column gap-3">
      ...
      <!-- Use text + numeric-only + strict 6 digits -->
      <input
...
// Generate a 6-digit numeric captcha (keeps leading zeros if any)
function generateCaptcha() {
  // slightly better randomness than Math.random() * 900000
...
async function submit() {
  // Clean and validate captcha input
  ...
  try {
    ...
    router.replace(me.role === 'admin' ? '/admin' : '/home')
  } catch (e) {
    alert(e?.message || 'Login failed. Please check your email and password.')
    // Regenerate captcha on failure to slow brute-force attempts
    generateCaptcha()
  }
}
</script>
